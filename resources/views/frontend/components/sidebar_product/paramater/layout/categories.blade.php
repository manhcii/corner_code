...
<div class="products-filter-item">
    ...
    <div id="filter-categories" class="collapse show">
        <ul class="products-filter-item-criteria" data-type="checkbox">
            ...
            <a href="{{route('frontend.page', ['taxonomy' => App\Consts::ROUTE_TAXONOMY['product'], 'alias' => $item->alias ?? ''])}}">
            <li>

                    <div class="checkbox" data-status="uncheck"></div>
                    <a href="{{route('frontend.page', ['taxonomy' => App\Consts::ROUTE_TAXONOMY['product'], 'alias' => $item->alias ?? ''])}}">
                <p class="text">
                    {{$item->json_params->name->$locale??$item->name}} <span class="quantity">({{$item->count_post}})</span>
                </p>
