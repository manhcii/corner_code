...
class CmsTaxonomy extends Model
{
    ...
    public static function getSqlTaxonomy($params = [])
    {
        ...
        if (!empty($params['count']) && $params['count'] == true) {
            ...
            $query->Join('tb_cms_relationships', 'tb_cms_taxonomys.id', '=', 'tb_cms_relationships.taxonomy_id');
        };
    ...
    public static function getTaxonomyByPostId($id, $type)
    {
        $taxonomy_id = CmsRelationship::where('object_id', $id)
                                      ->where('object_type',$type)
                                      ->first();
        if($taxonomy_id) {
            $taxonomy = CmsTaxonomy::findOrFail($taxonomy_id->taxonomy_id);
            if($taxonomy) {
                return $taxonomy;
            }
        }
